...
在微前端架构中，主应用和子应用之间的样式隔离是确保各应用独立运行且互不干扰的关键。本文档总结了常见的样式隔离方案，并说明本项目采用的推荐方案。
...
### 3. CSS 命名空间前缀
- **原理**：为应用样式添加特定前缀，如 `.main-app .container`
- **优点**：实现简单，易于理解和维护
- **缺点**：需要规范团队命名约定
- **适用场景**：全局样式隔离
...
通过以上方案的组合使用，本项目有效解决了主子应用之间的样式冲突问题：
1. 主应用的样式通过命名空间前缀避免全局污染
2. 子应用的样式通过 CSS Modules 实现局部化
3. qiankun 的样式隔离机制进一步确保了样式隔离
4. Scoped CSS 确保组件级别的样式隔离

## 最佳实践

1. **主应用优先使用命名空间前缀**：为主应用的全局样式添加特定前缀
2. **子应用优先使用 CSS Modules**：对于需要动态类名的组件，优先使用 CSS Modules
